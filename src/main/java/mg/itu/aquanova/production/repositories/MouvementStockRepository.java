...
public interface MouvementStockRepository extends JpaRepository<MouvementStock, Long> {
    ...
    List<MouvementStock> findTop10ByAliment_IdOrderByDateMouvementDesc(Long alimentId);

    @Query("""
        SELECT m
        FROM MouvementStock m
        WHERE (:debut IS NULL OR m.dateMouvement >= :debut)
          AND (:fin IS NULL OR m.dateMouvement <= :fin)
          AND (:alimentId IS NULL OR m.aliment.id = :alimentId)
          AND (:typeMvt IS NULL OR m.typeMouvement = :typeMvt)
